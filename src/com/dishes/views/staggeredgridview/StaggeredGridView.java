...
public class StaggeredGridView extends ViewGroup {
	...
	public boolean onInterceptTouchEvent( MotionEvent ev ) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if( index < 0 ) {
				Log.e( TAG, "onInterceptTouchEvent could not find pointer with id " + mActivePointerId + " - did StaggeredGridView receive an inconsistent "
						+ "event stream?" );
				return false;
			}
	...
	final LayoutRecord getNextRecordUp( int position, int span ) {
		...
		if( rec == null ) {
			...
			throw new IllegalStateException( "Invalid LayoutRecord! Record had span=" + rec.span + " but caller requested span=" + span + " for position="
					+ position );
		}
		int targetCol = -1;
		int bottomMost = Integer.MIN_VALUE;
	...
	public static class LayoutParams extends ViewGroup.LayoutParams {
		...
		public LayoutParams( int height ) {
			...
			if( this.height == MATCH_PARENT ) {
				Log.w( TAG, "Constructing LayoutParams with height FILL_PARENT - " + "impossible! Falling back to WRAP_CONTENT" );
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			...
			return "StaggereGridView.SavedState{" + Integer.toHexString( System.identityHashCode( this ) ) + " firstId=" + firstId + " position=" + position
					+ "}";
		}
