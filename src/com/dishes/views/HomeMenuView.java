...
public class HomeMenuView extends ViewGroup {
	...
	private final float WIDTH_RATE = 0.7f;
	/**
	 * 主布局
	 */
	 ...
	 * 左边菜单布局
	 */
	private SlidingMenuView left_show_view;
	...
	private int move_main;// 移动距离
	private int move_left;// 移动的最后距离
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage( Message msg ) {
			...
			synchronized( HomeMenuView.this ) {
	...
	public HomeMenuView( Context context, AttributeSet attrs, int defStyle ) {
	...
	public void initView() {
		...
		this.screen_w = CommonMethod.getWindowSizeW( context );
		this.screen_h = CommonMethod.getWindowSizeH( context );
		this.move_main =CommonMethod.getWindowSizeW( context )/2;
		this.move_left = CommonMethod.getWindowSizeW( context ) / 2;
		this.setKeepScreenOn( true );
		this.min_distance = ( int )( screen_w / 2.0 );
	}
	 ...
	 * @param currentTab
	 */
	public void setMainView( SlidingMenuView leftView, BaseView mainView, int curTab ) {
	...
	public void move( int start ) {
		...
		if( now_state == MAIN ) {
			...
			main_show_view.getView().layout( start, 0, start + screen_w, screen_h );
		} else {
	...
	protected void onMeasure( int widthMeasureSpec, int heightMeasureSpec ) {
		...
		left_show_view.getView().measure( MeasureSpec.UNSPECIFIED, heightMeasureSpec );
			...
			case MotionEvent.ACTION_MOVE:// 拖动时
				...
				if( !isMoved ) {
					if( Math.abs( last_y - start_y ) > Math.abs( last_x - start_x ) ) {
						...
						/*
						 * if (Math.abs(last_x - start_x) > 10) { isMoved =
						 * true; }
						 */
						if( last_x - start_x > 100 || ( Math.abs( last_x - start_x ) >100 && now_state == LEFT ) ) {
							isMoved = true;
						}
					}
				}
				...
				if( isMoved ) {
					...
					if( this.touch_state == TOUCH_STATE_MOVING ) {
						...
						if( Math.abs( last_x - start_x ) > 100 ) {
	...
	public SlidingMenuView getLeft_show_view() {
	...
	public void showHideLeftMenu() {
		...
		if( now_state == HomeMenuView.MAIN ) {
			moveToLeft( true );
		} else {
			moveToMain( true, 0 );
		}
