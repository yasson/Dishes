...
import static com.dishes.views.flipview.TextureUtils.d2r;
import static com.dishes.views.flipview.TextureUtils.isValidTexture;
import static com.dishes.views.flipview.TextureUtils.toFloatBuffer;
import static com.dishes.views.flipview.TextureUtils.toShortBuffer;
		...
		if( angle > 0 ) {
			...
			if( axis == AXIS_TOP ) {
				if( orientationVertical ) {
					...
					shadowVertices = new float[] { cardVertices[ X_00 ], cardVertices[ Y_01 ] + h, z, cardVertices[ X_01 ], cardVertices[ Y_01 ], 0f,
							cardVertices[ X_11 ], cardVertices[ Y_11 ], 0f, cardVertices[ X_10 ], cardVertices[ Y_01 ] + h, z };
				} else { // horizontal
					w = ( cardVertices[ X_10 ] - cardVertices[ X_00 ] ) * ( 1.0f - cos( d2r( angle ) ) );
					z = ( cardVertices[ X_10 ] - cardVertices[ X_00 ] ) * sin( d2r( angle ) );
					shadowVertices = new float[] { cardVertices[ X_10 ] - w, cardVertices[ Y_00 ], z, cardVertices[ X_11 ] - w, cardVertices[ Y_01 ], z,
							cardVertices[ X_11 ], cardVertices[ Y_11 ], 0f, cardVertices[ X_10 ], cardVertices[ Y_10 ], 0f };
				}
			} else {
				if( orientationVertical ) {
					h = ( cardVertices[ Y_00 ] - cardVertices[ Y_01 ] ) * ( 1f - cos( d2r( angle ) ) );
					z = ( cardVertices[ Y_00 ] - cardVertices[ Y_01 ] ) * sin( d2r( angle ) );
					shadowVertices = new float[] { cardVertices[ X_00 ], cardVertices[ Y_00 ], 0f, cardVertices[ X_01 ], cardVertices[ Y_00 ] - h, z,
							cardVertices[ X_11 ], cardVertices[ Y_00 ] - h, z, cardVertices[ X_10 ], cardVertices[ Y_00 ], 0f };
				} else { // horizontal
					w = ( cardVertices[ X_10 ] - cardVertices[ X_00 ] ) * ( 1f - cos( d2r( angle ) ) );
					z = ( cardVertices[ X_10 ] - cardVertices[ X_00 ] ) * sin( d2r( angle ) );
					shadowVertices = new float[] { cardVertices[ X_00 ], cardVertices[ Y_00 ], 0f, cardVertices[ X_01 ], cardVertices[ Y_01 ], 0f,
							cardVertices[ X_00 ] + w, cardVertices[ Y_11 ], z, cardVertices[ X_01 ] + w, cardVertices[ Y_10 ], z };
				}
			}
